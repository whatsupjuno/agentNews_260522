...
function NewsFeedScreen() {
  ...
  const handlePillTap = useCallback((pillPos) => {
    ...
    setProgress((p) => {
      ...
      if (pillPos === expected) return p + 1;
      if (pillPos === TRIGGER_SEQUENCE[0]) return 1;
      return 0;
    });
  }, [armed]);
...
function SequencePeek({ progress, armed }) {
  ...
  // 시퀀스 진행 상황을 카테고리 라벨로 표시 (위치 → 카테고리 이름)
  const posToLabel = Object.fromEntries(CATEGORIES.map(c => [c.pos, c.label]));
  const taken = TRIGGER_SEQUENCE.slice(0, progress).map(p => `${p}·${posToLabel[p]}`);
  const nextPos = TRIGGER_SEQUENCE[progress];
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontWeight: 700 }}>
        ...
        <span style={{ marginLeft: 'auto', opacity: 0.7 }}>seq {progress}/4</span>
      </div>
      ...
      <div style={{ marginTop: 2, opacity: 0.7, fontSize: 10 }}>
        {armed
          ? (nextPos ? `next: pill #${nextPos} (${posToLabel[nextPos]})` : 'complete')
